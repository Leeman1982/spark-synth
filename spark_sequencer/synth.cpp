...
// ─── Wave type mapping ────────────────────────────────────────────────────────
// AMY wave_t: SINE=0, PULSE=1, SAW_DOWN=2, SAW_UP=3, TRIANGLE=4, NOISE=5

static uint8_t waveToAmy(WaveType wt) {
    switch (wt) {
        case WaveType::SINE:       return SINE;
        case WaveType::TRIANGLE:   return TRIANGLE;
        case WaveType::SAW:        return SAW_DOWN;
        case WaveType::SQUARE:     return PULSE;
        case WaveType::PULSE:      return PULSE;
        case WaveType::WAVE_NOISE: return NOISE;
        default:                   return SINE;
    }
}
...
void SynthEngine::begin(float sr) {
    ...
    // Apply default params to configure AMY synth channel.
    setParams(_p);
}
...
void SynthEngine::configMode() {
    ...
    if (_p.mode == SynthMode::DUAL_OSC || _p.mode == SynthMode::BASS) {
        ...
        e.patch_number   = 1024;
        e.num_voices     = 8;
        e.oscs_per_voice = 3;  // primary + secondary + noise
        amy_add_event(&e);
        ...
        e.synth          = AMY_SYNTH_CH;
        e.osc            = 0;
        e.wave           = (_p.mode == SynthMode::BASS)
                               ? PULSE
                               : waveToAmy(_p.osc1Wave);
        e.amp_coefs[0]   = 0.0f;
        e.amp_coefs[1]   = _p.osc1Level * 0.6f; // velocity
        e.amp_coefs[2]   = 1.0f;                 // EG0 amplitude
        e.chained_osc    = 1;
        amy_add_event(&e);

        // 4. Osc 1 — secondary / detuned wave, chained from osc 0
        ...
        e.amp_coefs[2]   = 1.0f;                 // EG0
        e.freq_coefs[0]  = 0.0f;
        e.freq_coefs[1]  = detuneRatio;           // frequency ratio vs note pitch
        e.chained_osc    = 2;
        amy_add_event(&e);

        // 5. Osc 2 — noise layer (muted unless noiseLevel > 0)
        e = amy_default_event();
        e.synth          = AMY_SYNTH_CH;
        e.osc            = 2;
        e.wave           = NOISE;
        ...
        // ── Patch-based mode (Juno / FM / Keys / Pad) ────────────────────────
        e = amy_default_event();
        e.synth = AMY_SYNTH_CH;
        ...
        switch (_p.mode) {
            case SynthMode::JUNO:
                e.num_voices   = 6;
                e.patch_number = _p.junoPatch;          // AMY patches 0-127 (Juno)
                break;

            case SynthMode::FM:
                e.num_voices   = 8;
                e.patch_number = (uint16_t)_p.fmPatch + 128; // AMY patches 128-255 (DX7)
                break;
            ...
            case SynthMode::PAD:
                e.num_voices   = 8;
                e.patch_number = (_p.junoPatch < 127) ? _p.junoPatch : 0;
                break;
...
void SynthEngine::sendAdsr() {
    // FM/DX7 patches use built-in operator envelopes — skip override
    if (_p.mode == SynthMode::FM) return;
...
void SynthEngine::sendFilter() {
    // FM patches manage their own filter — skip override
    if (_p.mode == SynthMode::FM) return;
    ...
    // AMY filter types: 0=none, 1=LP, 2=BP, 3=HP, 4=double-LP
    switch (_p.filterMode) {
        ...
        case FilterMode::NOTCH:    e.filter_type = 1; break;  // fallback to LP
    }
...
void SynthEngine::sendEffects() {
    if (_p.reverbAmt > 0.01f) {
        config_reverb(_p.reverbAmt * 2.0f, 0.85f, 0.5f, 3000.0f);
    } else {
        config_reverb(0.0f, 0.85f, 0.5f, 3000.0f);
    }

    if (_p.delayTime > 0.01f) {
        float delayMs = _p.delayTime * 1000.0f;
        config_echo(_p.delayFeedback, delayMs, 3000.0f, _p.delayFeedback * 0.8f, 0.0f);
    } else {
        config_echo(0.0f, 500.0f, 3000.0f, 0.0f, 0.0f);
    }

    // Master volume on synth channel
    amy_event e = amy_default_event();
    e.synth  = AMY_SYNTH_CH;
    e.volume = _p.masterVol * 4.0f;
    amy_add_event(&e);
}
...
void SynthEngine::setLFORate(float hz) {
    _p.lfoRate = hz;
}

void SynthEngine::setLFODepth(float d) {
    _p.lfoDepth = d;
}
